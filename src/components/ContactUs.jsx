...
const ContactUs = () => {
  ...
  return (
    <section
      className="min-h-screen flex flex-col items-center justify-center px-6 py-20 font-sans relative "
      style={{
    ...
    >
      ...
      <div className="relative max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 gap-20 shadow-2xl rounded-3xl bg-white/70 backdrop-blur-md p-6 px-32 mt-20 z-10">
        
        {/* Left Side - Info */}
        <div className="flex flex-col justify-center space-y-10">
          <h1 className="text-6xl font-extrabold tracking-tight text-gray-900 leading-tight drop-shadow-md">
            Let's Connect
          </h1>
          <p className="text-lg text-gray-700 max-w-lg leading-relaxed tracking-wide">
        ...
        <div className="relative flex flex-col justify-between">
          {submitted ? (
            <div className="flex flex-col items-center justify-center h-[] space-y-6 p-8">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-20 h-20 text-green-500 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 bg-white/90 rounded-2xl p-10 shadow-lg backdrop-blur-sm">
              ...
              <button
                type="submit"
                className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white text-lg font-semibold rounded-xl shadow-lg transition"
              >
                Send Message
              </button>
            </form>
          ...
          {/* Map */}
          <div className="mt-12 w-full h-64 rounded-xl overflow-hidden shadow-lg">
            <iframe
...
const FloatingInput = ({ label, type, name, value, onChange, required }) => (
  <div className="relative z-0 w-full group">
    <input
      ...
      value={value}
      onChange={onChange}
      className="block py-3 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-indigo-600 peer"
      placeholder=" "
    />
    <label
      htmlFor={name}
...
const FloatingTextarea = ({ label, name, value, onChange, rows, required }) => (
  <div className="relative z-0 w-full group">
    <textarea
      ...
      value={value}
      onChange={onChange}
      className="block py-3 px-0 w-full text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-indigo-600 resize-none peer"
      placeholder=" "
    />
    <label
      htmlFor={name}
